...
class ChatConversationListConsumer(WebsocketConsumer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.new_conversation_name = None
    ...
    def connect(self):
        ...
        try:
            ...
            self.new_conversation_name, conversation_instance = get_or_create_new_conversation(user1=int(user_id), user2=int(chat_id))
...
class ChatGroupConversationListConsumer(WebsocketConsumer):
    """Consumer for getting conversations from a particular ticket"""
    ...
    def connect(self):
        ...
        try:
            ...
            group_id = str(query_string.get("group_id")[0])
            if not (user_id or group_id or is_valid_token):
                self.disconnect("UNAUTHORIZED")
            
            self.conversation_name = f"group_chat_{group_id}"
            self.new_conversation_name = f"new_group_chat_{group_id}"
            ...
            self.disconnect(close_code=f"Group Conversation socket was disconnected due to , {str(e)}")
